...
export async function POST(req: NextRequest) {
  try {
    ...
    const transaction = await TransactionModel.findById(ourTransactionId);
    ...
    if (status === 'completed' || status === 'success') { // Adjust status strings as per UddoktaPay docs
      ...
      transaction.description = transaction.description + ' (Completed via webhook)';
      ...
      if (user) {
        ...
        console.error(`[PaymentWebhook] User not found in MongoDB for UID: ${user_uid}. Balance not updated.`);
      }
      ...
      transaction.gateway_transaction_id = gateway_transaction_id;
      transaction.description = transaction.description + ` (Status: ${status} via webhook)`;
      await transaction.save();
